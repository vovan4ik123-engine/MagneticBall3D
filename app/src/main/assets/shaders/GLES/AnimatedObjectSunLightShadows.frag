...
void main()
{
    ...
    float specular = pow(max(dot(fragToCameraDir, reflectDir), 0.0f), 64.0f) * specularLightStrength;
    ...
    {
        shadowMultiplier = closestDepth < currentDepth - 0.0003f ? 0.0f : 1.0f; // currentDepth - 0.0... is bias offset
    }
